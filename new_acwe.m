function[ u] = new_acwe(u0,Img,  timestep,...
...
u = u0;
[threshold I1]=maxentropie(Img);
 PriorP=mean(Img(Img>=threshold));


 PriorPB=mean(Img(Img<threshold));
 I=Img;
...
for k1=1:numIter
    ...
    c1 = mean(Img(inside_idx)); c2 = mean(Img(outside_idx));  %local
    I(outside_idx)=0;
   % figure,imshow(I,[]);
    ...
    G=K-(c11-c22);
    data_force = -DrcU.*(mu*G - v - lambda1*(Img-c1).^2 + lambda2*(Img-c2).^2);
    %introduce the distance regularation term:
    P=pc*(4*del2(u) - G);               %ref[2]
    u = u+timestep*(data_force+P); % gradient descent
